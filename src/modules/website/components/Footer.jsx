import React from 'react'
import { FaFacebookF, FaTwitter, FaInstagram, FaYoutube, FaLinkedinIn, FaMapMarkerAlt, FaPhone, FaEnvelope, FaChevronRight, FaTrophy, FaMusic } from 'react-icons/fa'
import logo from "@/assets/images/logo.png";
const quickLinks = [
  { label: 'Home',    href: '/' },
  { label: 'Members', href: '/members' },
  { label: 'Blogs',   href: '/blogs' },
  { label: 'Players', href: '/players' },
  { label: 'Contact', href: '/contact' },
  { label: 'Join Us', href: '/join' },
]

const programs = [
  { label: 'Talent Hunt',      href: '/programs/talent-hunt' },
  { label: 'Financial Aid',    href: '/programs/financial-aid' },
  { label: 'Academy Tie-ups',  href: '/programs/academy' },
  { label: 'Girl Empowerment', href: '/programs/girl-empowerment' },
  { label: 'Mentorship',       href: '/programs/mentorship' },
  { label: 'Sports Events',    href: '/programs/events' },
]

const contactInfo = [
  { icon: <FaMapMarkerAlt />, value: '123 Sports Complex, Sector 12, New Delhi – 110001' },
  { icon: <FaPhone />,        value: '+91 98765 43210' },
  { icon: <FaEnvelope />,     value: 'info@sportforce.org' },
  { icon: <FaEnvelope />,     value: 'CIN NUmber' },
]

const socials = [
  { icon: <FaFacebookF />,  href: '#' },
  { icon: <FaTwitter />,    href: '#' },
  { icon: <FaInstagram />,  href: '#' },
  { icon: <FaYoutube />,    href: '#' },
  { icon: <FaLinkedinIn />, href: '#' },
]

const Footer = () => {

  return (
    <>
      <style>{`
        /* Footer link hover */
        .f-link {
          transition: color .2s ease, transform .2s ease;
          cursor: pointer;
          display: flex; align-items: center; gap: 6px;
          text-decoration: none;
        }
        .f-link:hover { color: #F05A1A !important; transform: translateX(4px); }
        .f-link:hover .f-chevron { color: #F05A1A !important; }

        /* Social icon */
        .soc-btn {
          transition: all .25s cubic-bezier(.16,1,.3,1);
          cursor: pointer;
        }
        .soc-btn:hover {
          background: linear-gradient(135deg,#F05A1A,#FF7D42) !important;
          border-color: transparent !important;
          color: #fff !important;
          transform: translateY(-3px) scale(1.1);
          box-shadow: 0 8px 20px rgba(240,90,26,.4) !important;
        }

        /* Contact item hover */
        .c-item {
          transition: transform .2s ease;
        }
        .c-item:hover { transform: translateX(4px); }
        .c-item:hover .c-icon { color: #FF7D42 !important; }

        /* Logo hover */
        .footer-logo-icon {
          transition: transform .3s ease, box-shadow .3s ease;
        }
        .footer-logo-icon:hover {
          transform: scale(1.1) rotate(5deg);
          box-shadow: 0 10px 28px rgba(240,90,26,.5) !important;
        }

        /* Music note spin */
        @keyframes spin {
          from { transform: rotate(0deg); }
          to   { transform: rotate(360deg); }
        }
        .music-spin { animation: spin 3s linear infinite; }

        /* Top border shimmer */
        @keyframes shim { 0%{background-position:0% 50%} 100%{background-position:200% 50%} }
        .footer-shimmer {
          height: 3px;
          background: linear-gradient(90deg,#F05A1A,#FFAD5C,#F05A1A,#FFAD5C);
          background-size: 300% 100%;
          animation: shim 3s linear infinite;
        }
      `}</style>

      <footer
        className="footer-wrap"
        style={{ background: 'linear-gradient(160deg,#0B1E4B 0%,#0d2258 50%,#0B1E4B 100%)' }}
      >
        {/* Shimmer top border */}
        <div className="footer-shimmer" />

        {/* ── Main Footer Content ── */}
        <div
          className="!max-w-[1280px] !mx-auto !grid !grid-cols-1 sm:!grid-cols-2 lg:!grid-cols-4 !gap-10"
          style={{ padding: '60px 28px 48px' }}
        >

          {/* ── Col 1 — Brand ── */}
          <div>
            {/* Logo */}
       {/* LOGO */}
                <a
                  href="/"
                  className="flex items-center gap-3 no-underline group"
                  style={{ textDecoration: "none" }}
                >
                  <img src={logo} alt="logo" className="w-full max-w-2/3" /></a>

            {/* About text */}
            <p
              className="!mb-6 !mt-0"
              style={{
                fontSize: 13.5,
                color: 'rgba(255,255,255,.55)',
                lineHeight: 1.8,
                maxWidth: 240,
              }}
            >
              A non-profit organization dedicated to identifying, nurturing and empowering talented athletes across India. Building champions, changing lives.
            </p>

            {/* Social Icons */}
            <div className="!flex !items-center !gap-2 !flex-wrap">
              {socials.map((s, i) => (
                <a
                  key={i}
                  href={s.href}
                  className="soc-btn !flex !items-center !justify-center !rounded-xl !no-underline"
                  style={{
                    width: 38, height: 38,
                    background: 'rgba(255,255,255,.08)',
                    border: '1px solid rgba(255,255,255,.12)',
                    color: 'rgba(255,255,255,.7)',
                    fontSize: 14,
                  }}
                >
                  {s.icon}
                </a>
              ))}
            </div>
          </div>

          {/* ── Col 2 — Quick Links ── */}
          <div>
            <h4
              className="!mt-0 !mb-5"
              style={{
                fontSize: 11, fontWeight: 800,
                letterSpacing: '2.5px', textTransform: 'uppercase',
                color: '#fff',
              }}
            >
              Quick Links
            </h4>
            <ul className="!m-0 !p-0 !list-none !flex !flex-col !gap-3">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="f-link"
                    style={{ fontSize: 13.5, color: 'rgba(255,255,255,.55)', fontWeight: 500 }}
                  >
                    <FaChevronRight className="f-chevron" style={{ fontSize: 9, color: '#F05A1A', flexShrink: 0 }} />
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* ── Col 3 — Programs ── */}
          <div>
            <h4
              className="!mt-0 !mb-5"
              style={{
                fontSize: 11, fontWeight: 800,
                letterSpacing: '2.5px', textTransform: 'uppercase',
                color: '#fff',
              }}
            >
              Programs
            </h4>
            <ul className="!m-0 !p-0 !list-none !flex !flex-col !gap-3">
              {programs.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="f-link"
                    style={{ fontSize: 13.5, color: 'rgba(255,255,255,.55)', fontWeight: 500 }}
                  >
                    <FaChevronRight className="f-chevron" style={{ fontSize: 9, color: '#F05A1A', flexShrink: 0 }} />
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* ── Col 4 — Contact Info ── */}
          <div>
            <h4
              className="!mt-0 !mb-5"
              style={{
                fontSize: 11, fontWeight: 800,
                letterSpacing: '2.5px', textTransform: 'uppercase',
                color: '#fff',
              }}
            >
              Contact Info
            </h4>
            <div className="!flex !flex-col !gap-4">
              {contactInfo.map((item, i) => (
                <div key={i} className="c-item !flex !items-start !gap-3">
                  <span
                    className="c-icon !flex-shrink-0 !mt-0.5"
                    style={{ color: '#F05A1A', fontSize: 14, transition: 'color .2s ease' }}
                  >
                    {item.icon}
                  </span>
                  <span style={{ fontSize: 13.5, color: 'rgba(255,255,255,.55)', lineHeight: 1.6 }}>
                    {item.value}
                  </span>
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* ── Copyright Bar ── */}
        <div
          style={{ borderTop: '1px solid rgba(255,255,255,.08)' }}
        >
          <div
            className="!max-w-[1200px] !mx-auto !flex !items-center !justify-between !flex-wrap !gap-3"
            style={{ padding: '18px 28px' }}
          >
            <p
              className="!m-0"
              style={{ fontSize: 12.5, color: 'rgba(255,255,255,.35)', fontWeight: 400 }}
            >
               © {new Date().getFullYear()} UDIISA NGO. All rights reserved.
            </p>

            <div
              className="!flex !items-center !gap-2"
              style={{ fontSize: 12.5, color: 'rgba(255,255,255,.35)' }}
            >
              Made with <span style={{ color: '#F05A1A', fontSize: 13 }}>❤️</span> by UDIISA Web Team
            </div>
          </div>
        </div>
      </footer>
    </>
  )
}

export default Footer